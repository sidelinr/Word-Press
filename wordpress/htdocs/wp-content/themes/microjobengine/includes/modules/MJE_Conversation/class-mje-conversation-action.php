...
//            Add info meta if reject custom order
    ...
    */
    ...
    public function check_pending_account($result, $request) {
        if((isset($request['page']) && $request['page'] == 'mjob_order') || $request['type'] == 'custom_order') {
            $result = array(
                'success' => true,
                'msg' => __("Successful.", 'enginethemes')
            );
    ...
    public function add_opening_message($result, $request) {
        ...
        if( ! $request['opening_message'] || $request['opening_message'] == '')
            return;
        ...
        $data_post = array(
            ...
            'post_title' => sprintf(__('First Message from mJob %s of %s', 'enginethemes'), $request['mjob_name'], $request->mjob_author_name)
        );
