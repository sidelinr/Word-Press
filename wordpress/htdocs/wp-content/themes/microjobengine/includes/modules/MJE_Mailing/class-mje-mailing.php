...
//        Get mjob name
    ...
    public function filter_order_placeholder($message, $order) {
        ...
        if(!empty($custom_offer_id)) {
            ...
            $message = str_ireplace('[extra_price]', mje_format_price($order->amount - $offer_price), $message);
        } else {
            // Filter for normal order
            $message = str_ireplace('[mjob_price]', mje_format_price($mjob_price), $message);
            $message = str_ireplace('[extra_price]', mje_format_price($order->amount - $mjob_price), $message);
        }
